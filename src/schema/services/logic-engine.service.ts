    ...
    private compareValues(
        ...
        switch (operator) {
            ...
            case ComparisonOperator.GREATER_THAN_OR_EQUALS:
                return (
                    this.toNumber(answerValue) >= this.toNumber(compareValue)
                );
            ...
            case ComparisonOperator.STARTS_WITH:
                return this.toStr(answerValue).startsWith(
                    this.toStr(compareValue),
                );

            case ComparisonOperator.ENDS_WITH:
                return this.toStr(answerValue).endsWith(
                    this.toStr(compareValue),
                );
    ...
    private validateRule(
        ...
        if (!rule.action) {
            ...
            if (!targetId) {
                ...
                errors.push(
                    `${prefix}.action: Invalid page target ID: ${targetId}`,
                );
            }
        }
    ...
    private validateConditionReferences(
        ...
        if (isConditionGroup(condition)) {
            ...
            if (!condition.questionId) {
                ...
                errors.push(
                    `${path}: Invalid question ID: ${condition.questionId}`,
                );
            }
        }
