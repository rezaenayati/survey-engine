...
export class ResponsesService {
  ...
  async complete(ctx: RequestContext, id: string): Promise<Response> {
    ...
    const missingLogicRequired = logicResult.requiredQuestions.filter((qId) => {
      const answer = response.answersJson[qId];
      return answer === undefined || answer === null || answer === '';
    });

    if (visibleQuestionErrors.length > 0 || missingLogicRequired.length > 0) {
    ...
    const survey = await this.surveysService.findOne(ctx, completed.surveyId);
    this.webhookService.fire(survey.settings, {
  ...
  async validate(
    ...
    return {
      valid: visibleQuestionErrors.length === 0 && missingRequired.length === 0,
      errors: visibleQuestionErrors,
      missingRequired,
