...
import { startTestDatabase, stopTestDatabase, getDataSource } from '../helpers/test-database';
...
describe('Responses API (e2e)', () => {
  ...
  describe('POST /responses/start', () => {
    it('creates a response row in the DB pinned to the active version', async () => {
      ...
      const row = await responseRepo.findOne({ where: { id: res.body.id } });
    ...
    it('creates anonymous response (null respondentId) when no X-User-ID', async () => {
      ...
      const row = await getDataSource().getRepository(Response).findOne({ where: { id: res.body.id } });
  ...
  describe('GET /responses', () => {
    it('returns responses belonging to the authenticated respondent', async () => {
      ...
      expect(res.body.data.every((r: Response) => r.respondentId === 'respondent-1')).toBe(true);
    });
    ...
    it('returns all responses for a survey when the survey owner queries by surveyId', async () => {
      ...
      expect(res.body.data.every((r: Response) => r.surveyId === surveyId)).toBe(true);
    });
  ...
  describe('GET /responses/:id', () => {
    it('returns the response that exists in the DB', async () => {
      const row = await getDataSource().getRepository(Response).findOne({ where: { id: responseId } });
      expect(row).not.toBeNull();
    ...
    it('returns 404 for an ID not in the DB', async () => {
      ...
      expect(await getDataSource().getRepository(Response).findOne({ where: { id: missingId } })).toBeNull();
  ...
  describe('PATCH /responses/:id', () => {
    it('merges answers into the DB row and sets status to in_progress', async () => {
      ...
      const before = await responseRepo.findOne({ where: { id: responseId } });
      ...
      const after = await responseRepo.findOne({ where: { id: responseId } });
      expect(after!.status).toBe('in_progress');
      expect((after!.answersJson as Record<string, unknown>).q1).toBe('Alice');
    });
    ...
    it('merges additional answers without overwriting existing ones', async () => {
      ...
      const row = await responseRepo.findOne({ where: { id: responseId } });
      expect((row!.answersJson as Record<string, unknown>).q1).toBe('Alice'); // preserved
    ...
    it('returns 403 when a different user tries to update', async () => {
      const before = await getDataSource().getRepository(Response).findOne({ where: { id: responseId } });
      ...
      const after = await getDataSource().getRepository(Response).findOne({ where: { id: responseId } });
  ...
  describe('GET /surveys/:id/analytics — after completing a response', () => {
    it('reports the completed response in summary stats', async () => {
      const responseRepo = getDataSource().getRepository(Response);
      const completed = await responseRepo.count({ where: { surveyId, status: 'completed' as never } });
  ...
  describe('DELETE /responses/:id', () => {
    it('removes the row from the DB', async () => {
      ...
      expect(await responseRepo.findOne({ where: { id } })).not.toBeNull();
  ...
  describe('version pinning', () => {
    it('in-flight response stays pinned to version it started on after re-publish', async () => {
      ...
      const row = await getDataSource().getRepository(Response).findOne({ where: { id: startRes.body.id } });
      expect(row!.surveyVersionId).toBe(pinnedVersionId);

      // Survey now has a new active version
      const survey = await getDataSource().getRepository(Survey).findOne({ where: { id: surveyId } });
