...
describe('Surveys API (e2e)', () => {
  ...
  describe('POST /surveys', () => {
    it('creates a draft survey and persists it to the DB', async () => {
      ...
      const before = await surveyRepo.findOne({ where: { name: 'Customer Survey' } });
      ...
      const row = await surveyRepo.findOne({ where: { id: res.body.id } });
  ...
  describe('GET /surveys', () => {
    it('lists surveys owned by the authenticated user', async () => {
      ...
      expect(res.body.data.every((s: Survey) => s.createdBy === 'user-1')).toBe(true);
    });
    ...
    it('returns only published surveys for unauthenticated caller', async () => {
      ...
      expect(res.body.data.every((s: Survey) => s.status === 'published')).toBe(true);
    });
  });
  ...
  describe('GET /surveys/:id', () => {
    it('returns the survey that exists in the DB', async () => {
      ...
      const row = await getDataSource().getRepository(Survey).findOne({ where: { id: surveyId } });
      expect(row).not.toBeNull();
    ...
    it('returns 404 for an ID that does not exist in the DB', async () => {
      ...
      const row = await getDataSource().getRepository(Survey).findOne({ where: { id: missingId } });
      expect(row).toBeNull();

      await request(app.getHttpServer()).get(`/surveys/${missingId}`).expect(404);
    });
  });
  ...
  describe('PATCH /surveys/:id', () => {
    it('updates the survey name and persists the change to the DB', async () => {
      ...
      // Pre-condition
      const before = await surveyRepo.findOne({ where: { id: surveyId } });
  ...
  describe('POST /surveys/:id/publish', () => {
    it('publishes the survey, creates a version row, and updates activeVersionId in the DB', async () => {
      ...
      const versionsBefore = await versionRepo.find({ where: { surveyId } });
      ...
      // DB: status updated
      const surveyAfter = await surveyRepo.findOne({ where: { id: surveyId } });
    ...
    it('creates a second version row on re-publish', async () => {
      ...
      const versions = await versionRepo.find({ where: { surveyId }, order: { versionNumber: 'ASC' } });
  ...
  describe('GET /surveys/:id/runtime', () => {
    it('returns the active published version schema', async () => {
      ...
      const version = await getDataSource().getRepository(SurveyVersion).findOne({ where: { id: survey!.activeVersionId! } });
  ...
  describe('GET /surveys/:id/validate', () => {
    it('validates draft schema — no DB write', async () => {
      const before = await getDataSource().getRepository(Survey).findOne({ where: { id: surveyId } });
      ...
      const after = await getDataSource().getRepository(Survey).findOne({ where: { id: surveyId } });
  ...
  describe('DELETE /surveys/:id', () => {
    it('deletes the survey and removes the row from the DB', async () => {
      ...
      // API: 404
      await request(app.getHttpServer()).get(`/surveys/${id}`).expect(404);
    });
    ...
    it('returns 403 when a different user tries to delete', async () => {
      ...
      // DB: row still exists
      expect(await surveyRepo.findOne({ where: { id: surveyId } })).not.toBeNull();
    });
  });
});
