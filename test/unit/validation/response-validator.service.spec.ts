...
describe('ResponseValidatorService', () => {
  ...
  describe('validateAnswer — radiogroup / dropdown', () => {
    const choiceQ = { id: 'q', type: 'radiogroup', title: 'Q', isRequired: false, choices: [{ value: 'a' }, { value: 'b' }] };
  ...
  describe('validateAnswer — checkbox', () => {
    const multiQ = { id: 'q', type: 'checkbox', title: 'Q', isRequired: false };
  ...
  describe('validateAnswer — rating', () => {
    const ratingQ = { id: 'q', type: 'rating', title: 'Q', isRequired: false, rateMin: 1, rateMax: 5 };
    ...
    it('uses default range 1-5 when rateMin/rateMax not set', () => {
      const q = { id: 'q', type: 'rating', title: 'Q', isRequired: false };
  ...
  describe('validateAnswer — boolean', () => {
    const boolQ = { id: 'q', type: 'boolean', title: 'Q', isRequired: false };

    it('accepts true', () => expect(service.validateAnswer(boolQ, true).valid).toBe(true));
    it('accepts false', () => expect(service.validateAnswer(boolQ, false).valid).toBe(true));
    it('accepts string "true"', () => expect(service.validateAnswer(boolQ, 'true').valid).toBe(true));
    it('accepts string "false"', () => expect(service.validateAnswer(boolQ, 'false').valid).toBe(true));
  ...
  describe('validateAnswer — matrix', () => {
    const matrixQ = { id: 'q', type: 'matrix', title: 'Q', isRequired: false };
  ...
  describe('validateAnswer — signaturepad', () => {
    it('rejects non-string', () => {
      const q = { id: 'q', type: 'signaturepad', title: 'Q', isRequired: false };
  ...
  describe('validateAnswer — ranking', () => {
    it('rejects non-array', () => {
      const q = { id: 'q', type: 'ranking', title: 'Q', isRequired: false };
  ...
  describe('SurveyJS validators', () => {
    it('regex validator — passes matching string', () => {
      const q = { id: 'q', type: 'text', title: 'Q', isRequired: false, validators: [{ type: 'regex', regex: '^[0-9]+$' }] };
      const r = service.validateAnswer(q, '12345');
      expect(r.valid).toBe(true);
    });

    it('regex validator — fails non-matching string', () => {
      const q = { id: 'q', type: 'text', title: 'Q', isRequired: false, validators: [{ type: 'regex', regex: '^[0-9]+$', text: 'Numbers only' }] };
    ...
    it('regex validator — silently skips invalid regex', () => {
      const q = { id: 'q', type: 'text', title: 'Q', isRequired: false, validators: [{ type: 'regex', regex: '[invalid' }] };
      const r = service.validateAnswer(q, 'any');
      expect(r.valid).toBe(true);
    });

    it('text validator — enforces minLength', () => {
      const q = { id: 'q', type: 'text', title: 'Q', isRequired: false, validators: [{ type: 'text', minLength: 5 }] };
      const r = service.validateAnswer(q, 'ab');
      expect(r.valid).toBe(false);
      expect(r.errors[0].code).toBe('MIN_LENGTH');
    });
    ...
    it('text validator — enforces maxLength', () => {
      const q = { id: 'q', type: 'text', title: 'Q', isRequired: false, validators: [{ type: 'text', maxLength: 3 }] };
      const r = service.validateAnswer(q, 'toolong');
      expect(r.valid).toBe(false);
      expect(r.errors[0].code).toBe('MAX_LENGTH');
    });
    ...
    it('email validator — passes valid email', () => {
      const q = { id: 'q', type: 'text', title: 'Q', isRequired: false, validators: [{ type: 'email' }] };
    ...
    it('numeric validator — passes numeric string', () => {
      const q = { id: 'q', type: 'text', title: 'Q', isRequired: false, validators: [{ type: 'numeric' }] };
  ...
  describe('validateResponse — edge cases', () => {
    it('passes when schema has no pages', () => {
      const result = service.validateResponse({} as never, { q1: 'answer' });
      expect(result.valid).toBe(true);
    });
    ...
    it('includes questionResults for answered questions', () => {
      ...
      expect(result.questionResults.some(r => r.questionId === 'name')).toBe(true);
    });
  });
});
