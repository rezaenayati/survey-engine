...
import { WebhookService, WebhookPayload } from '../../../src/webhooks/webhook.service';
...
describe('WebhookService', () => {
  ...
  describe('fire()', () => {
    ...
    it('fires a POST request to the configured URL', async () => {
      const settings: Partial<SurveySettings> = { webhookUrl: 'https://example.com/hook' };
      service.fire(settings as SurveySettings, samplePayload);
      await new Promise((r) => setTimeout(r, 10));
      ...
      expect(JSON.parse(options.body as string)).toMatchObject({ event: 'response.completed' });
    });
    ...
    it('delivers response.started event when included in webhookEvents', async () => {
      ...
      service.fire(settings as SurveySettings, { ...samplePayload, event: 'response.started' });
      await new Promise((r) => setTimeout(r, 10));
      expect(fetchMock).toHaveBeenCalledTimes(1);
    });
