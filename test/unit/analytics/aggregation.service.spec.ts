...
describe('AggregationService', () => {
    ...
    describe('buildAppliedFilters', () => {
        ...
        it('reflects answerFilters in the result', () => {
            const filters = [
                { questionId: 'q1', operator: 'equals' as never, value: 'yes' },
            ];
            const result = service.buildAppliedFilters({
    ...
    describe('buildBaseQuery', () => {
        it('always filters by surveyId', () => {
            ...
            expect(repoMock._qb.where).toHaveBeenCalledWith(
                'r.surveyId = :surveyId',
                { surveyId: 'survey-1' },
            );
        });
        ...
        it('does not apply date filters when absent', () => {
            service.buildBaseQuery(ctx, 'survey-1', {});
            const calls = (repoMock._qb.andWhere as jest.Mock).mock.calls.map(
                ([sql]: [string]) => sql,
            );
            expect(calls.some((c: string) => c.includes('startedAt'))).toBe(
                false,
            );
        });
        ...
        it('does not apply version filter for COMBINED mode', () => {
            ...
            expect(
                calls.some((c: string) => c.includes('surveyVersionId')),
            ).toBe(false);
        });
    ...
    describe('buildBaseQuery — answer filters', () => {
        ...
        const scalarCases: Array<{
        ...
        }> = [
            {
                ...
                label: 'EQUALS',
                expectedSql: '= :answerFilter0',
            },
            {
                operator: FilterOperator.CONTAINS,
                label: 'CONTAINS',
                expectedSql: 'ILIKE :answerFilter0',
            },
            {
        ...
        for (const { operator, label, expectedSql } of scalarCases) {
            it(`applies ${label} operator filter`, async () => {
                ...
                svc.buildBaseQuery(ctx, 'survey-1', {
                    answerFilters: [
                        { questionId: 'q1', operator, value: 'testval' },
                    ],
                });

                const allCalls = (
                    repo._qb.andWhere as jest.Mock
                ).mock.calls.map(([sql]: [string]) => sql);
                expect(
                    allCalls.some(
                        (s: string) =>
                            typeof s === 'string' && s.includes(expectedSql),
                    ),
                ).toBe(true);
            });
        }
        ...
        it('applies IN operator with array value', async () => {
            ...
            const allCalls = (repo._qb.andWhere as jest.Mock).mock.calls.map(
                ([sql]: [string]) => sql,
            );
            expect(
                allCalls.some((s: string) =>
                    s.includes('IN (:...answerFilter0)'),
                ),
            ).toBe(true);
        });
