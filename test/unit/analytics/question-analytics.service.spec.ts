...
function createQbMock(overrides: Record<string, jest.Mock> = {}) {
    const qb: Record<string, jest.Mock> = {
        ...
        getQuery: jest
            .fn()
            .mockReturnValue(
                'SELECT r.id FROM response r WHERE r.surveyId = :surveyId',
            ),
...
describe('QuestionAnalyticsService', () => {
    ...
    let aggregationService: jest.Mocked<
        Pick<AggregationService, 'buildBaseQuery' | 'calculateMedian'>
    >;
    ...
    describe('getRelevantVersions', () => {
        it('returns all versions in COMBINED mode', async () => {
            ...
            const result = await service.getRelevantVersions('survey-1', {
                versionMode: VersionMode.COMBINED,
            });
    ...
    describe('calculateQuestionAnalyticsDB — empty versions', () => {
        it('returns empty array when no versions provided', async () => {
            const result = await service.calculateQuestionAnalyticsDB(
                ctx,
                'survey-1',
                {},
                [],
    ...
    describe('calculateQuestionAnalyticsDB — question type dispatch', () => {
        beforeEach(() => {
            ...
            (baseQb.getRawOne as jest.Mock).mockResolvedValue({
                total: '5',
                answered: '4',
            });
        });
        ...
        it('retains questions from both versions', async () => {
                ...
                pages: [
                    {
                        name: 'p1',
                        elements: [
                            { name: 'q1', type: 'text', title: 'Q1 old' },
                        ],
                    },
                ],
            };
            const schemaV2 = {
                pages: [
                    {
                        name: 'p1',
                        elements: [{ name: 'q2', type: 'text', title: 'Q2' }],
                    },
                ],
            };
            ...
            const versions = [
                makeVersion(1, schemaV1),
                makeVersion(2, schemaV2),
            ];
            const result = await service.calculateQuestionAnalyticsDB(
                ctx,
                'survey-1',
                ...
                pages: [
                    {
                        name: 'p1',
                        elements: [
                            { name: 'old_q', type: 'text', title: 'Old Q' },
                        ],
                    },
                ],
            };
                ...
                pages: [
                    {
                        name: 'p1',
                        elements: [
                            { name: 'new_q', type: 'text', title: 'New Q' },
                        ],
                    },
                ],
            };
    ...
    describe('getTextResponses', () => {
        ...
        it('returns paginated text responses', async () => {
            ...
            const result = await service.getTextResponses(
                ctx,
                'survey-1',
                'q_text',
                { page: 1, limit: 20 },
        ...
        it('applies search filter when provided', async () => {
            ...
            await service.getTextResponses(ctx, 'survey-1', 'q_text', {
                search: 'hello',
            });

            const andWhereCalls = (baseQb.andWhere as jest.Mock).mock.calls.map(
                ([sql]: [string]) => sql,
            );
