...
const mockAggregationService = {
  ...
  calculateSummaryDB: jest.fn().mockResolvedValue({ totalResponses: 10, completionRate: 80, completedResponses: 8, avgCompletionTime: 60, medianCompletionTime: 55, responsesByStatus: {}, responsesToday: 1, responsesThisWeek: 5, versionsIncluded: [1] }),
  calculateFunnelDB: jest.fn().mockResolvedValue({ total: 10, started: 2, inProgress: 2, completed: 8, abandoned: 0, activeResponses: 4, staleResponses: 0, completionRate: 80, dropOffRate: 20, abandonmentRate: 0 }),
...
const mockQuestionAnalyticsService = {
  ...
  getTextResponses: jest.fn().mockResolvedValue({ items: [], total: 0, page: 1, limit: 20, totalPages: 0, hasMore: false }),
};
...
describe('AnalyticsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        { provide: QuestionAnalyticsService, useValue: mockQuestionAnalyticsService },
  ...
  describe('getAnalytics', () => {
    it('resolves the survey and asserts ownership before computing', async () => {
      ...
      expect(mockSurveysService.findOne).toHaveBeenCalledWith(ctx, 'survey-1');
      expect(mockSurveysService.assertOwner).toHaveBeenCalledWith(mockSurvey, ctx);
    });

    it('throws BadRequestException when versionMode is SPECIFIC but versionId is absent', async () => {
      await expect(
        service.getAnalytics(ctx, 'survey-1', { versionMode: VersionMode.SPECIFIC }),
    ...
    it('delegates to all three sub-services in parallel', async () => {
      ...
      expect(mockAggregationService.calculateSummaryDB).toHaveBeenCalled();
      ...
      expect(mockQuestionAnalyticsService.calculateQuestionAnalyticsDB).toHaveBeenCalled();
    });
  ...
  describe('getSummary', () => {
    it('throws NotFoundException (from SurveysService) when survey does not exist', async () => {
      mockSurveysService.findOne.mockRejectedValueOnce(new Error('Not found'));

      await expect(service.getSummary(ctx, 'bad-id', {})).rejects.toThrow('Not found');
    });
  ...
  describe('getTextResponses', () => {
    it('delegates to questionAnalyticsService', async () => {
      await service.getTextResponses(ctx, 'survey-1', 'q1', { page: 1, limit: 10 });

      expect(mockQuestionAnalyticsService.getTextResponses).toHaveBeenCalledWith(
        ctx, 'survey-1', 'q1', { page: 1, limit: 10 },
      );
    });
  });
  ...
  describe('exportAnalytics', () => {
    it('returns CSV data when format is "csv"', async () => {
      const result = await service.exportAnalytics(ctx, 'survey-1', {}, 'csv');
    ...
    it('returns JSON data when format is "json"', async () => {
      const result = await service.exportAnalytics(ctx, 'survey-1', {}, 'json');

      expect(mockExportService.convertToCSV).not.toHaveBeenCalled();
      expect(result.contentType).toBe('application/json');
      expect(result.filename).toMatch(/survey-analytics-survey-1-\d+\.json/);
    });
  });
});
