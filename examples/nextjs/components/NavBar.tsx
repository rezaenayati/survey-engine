    ...
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-40">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <Link href="/" className="font-semibold text-indigo-600 text-sm tracking-wide">
            SurveyEngine
          </Link>
          <div className="flex gap-1">
            <NavLink href="/" active={!isAdmin}>Surveys</NavLink>
        ...
        <div className="relative">
          ...
          {open && (
            <div className="absolute right-0 mt-1 w-44 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50">
              <p className="px-3 py-1.5 text-xs text-gray-400 font-medium uppercase tracking-wide">Switch user</p>
              {PRESET_USERS.map(u => (
                <button
                  key={u}
                ...
                >
                  ...
                  {u}
                  {u === user && <span className="ml-auto text-indigo-400">✓</span>}
                </button>
              ))}
            </div>
          )}
