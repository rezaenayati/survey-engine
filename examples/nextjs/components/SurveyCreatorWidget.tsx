 ...
 */
export function SurveyCreatorWidget({ surveyId, initialSchema, onSaved, onSaveError }: Props) {
  ...
  useEffect(() => {
    ...
    c.saveSurveyFunc = (saveNo: number, callback: (no: number, success: boolean) => void) => {
      fetch(`/api/surveys/${surveyId}`, {
        ...
        body: JSON.stringify({ schemaJson: c.JSON as Record<string, unknown> }),
      })
        .then(r => {
          if (r.ok) {
  ...
  // Creator is created once per surveyId — re-create if the survey changes
