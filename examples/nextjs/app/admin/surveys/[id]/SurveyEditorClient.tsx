...
const SurveyCreatorWidget = dynamic(
  () => import('@/components/SurveyCreatorWidget').then(m => m.SurveyCreatorWidget),
  {
    ssr: false,
    loading: () => (
...
export function SurveyEditorClient({ survey }: { survey: Survey }) {
  ...
  const [description, setDescription] = useState(survey.description ?? '' as string);
  ...
  async function publish() {
    if (!confirm('Publish this survey? Respondents will be able to take it immediately.')) return;
    setPublishing(true);
    try {
      const res = await apiFetch(`/api/surveys/${survey.id}/publish`, { method: 'POST' });
      if (!res.ok) throw new Error((await res.json()).message);
    ...
    <div className="flex flex-col" style={{ height: 'calc(100vh - 56px)' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 shrink-0 bg-white border-r border-gray-200 overflow-y-auto flex flex-col gap-4 p-4">
          ...
          {/* Validation result */}
          {validation && (
            <section className={`rounded-xl border p-3 text-xs ${validation.schemaValid && validation.logicValid ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
              <p className="font-semibold mb-1">
                {validation.schemaValid && validation.logicValid ? '✓ Schema is valid' : '✗ Validation failed'}
              </p>
              {validation.schemaErrors.map((e, i) => <p key={i}>• {String(e)}</p>)}
              {validation.logicErrors.map((e, i) => <p key={i}>• {e}</p>)}
            </section>
          )}
            ...
            {settingsOpen && (
              <div className="mt-3 space-y-2">
                ...
                <p className="text-xs text-gray-400">
                  survey-engine signs each <code className="bg-gray-100 px-0.5 rounded">response.completed</code> payload with HMAC-SHA256.
                </p>
                <button
                  onClick={saveMeta}
                  disabled={saving}
        ...
        <div className="flex-1 overflow-hidden">
          <SurveyCreatorWidget
            ...
            onSaveError={() => showToast('Auto-save failed — check survey-engine connection')}
          />
        </div>
      </div>
