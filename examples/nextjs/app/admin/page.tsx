...
export default async function AdminPage() {
  ...
  const { data: surveys } = await client.surveys.list({ limit: 100, sortBy: 'createdAt', sortOrder: 'DESC' });
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{surveys.length} survey{surveys.length !== 1 ? 's' : ''} total</p>
        </div>
        <Link
      ...
      {surveys.length === 0 ? (
        <div className="text-center py-24 bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <Link href="/admin/surveys/new" className="text-indigo-500 underline text-sm mt-1 block">
            ...
            <tbody className="divide-y divide-gray-100">
              {surveys.map(survey => (
                <tr key={survey.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5"><StatusBadge status={survey.status} /></td>
